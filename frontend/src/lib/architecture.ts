  ...
  tabs: [
    {
      ...
      body_es:
        'ImageLab escribe una imagen como parametros de muchas maneras y pregunta que hace una perturbacion de esos parametros. La editabilidad tiene forma de U. Alcanza su maximo en el polo de estructura disenada (transformadas ortonormales, diccionarios dispersos, primitivas geometricas), donde un parametro es un control local y exacto, y en el polo de variedad aprendida (latentes de VAE y difusion), donde un parametro es un control semantico pero enredado. Entre los polos, un empujon a un parametro crudo o densamente codificado solo produce ruido.\n\nCada pestana de la App es un punto de esta curva, y la pagina de Experimentos mide la localidad de una edicion de un parametro para ubicar cada familia en ella.',
    },
    {
      id: 'offline',
      en: 'Offline bake',
      ...
      body_es:
        'Las representaciones pesadas se hornean una vez, offline, con un pipeline determinista de Python (imglab.methods) en su propio entorno. Aprende una base KLT por parches y diccionarios dispersos, entrena una pequena red sinusoidal (SIREN) por imagen, decodifica caminatas de interpolacion latente con el VAE de Stable Diffusion, genera tiras de difusion con SD-Turbo, ajusta primitivas geometricas y finalmente mide el benchmark entre familias. Cada uno escribe un indice compacto y sus arreglos bajo data/derived, que se versiona en el repo.\n\nLos artefactos son las entradas desplegables: el horneado es reproducible desde el repo pero nunca corre en tiempo de peticion.',
    },
    {
      id: 'live',
      ...
      body_es:
        'Las pestanas de transformada, marco, primitivas, simbolica y campo neuronal calculan en tu navegador, sin llamada al servidor. La imagen seleccionada (o tu carga) se decodifica a planos de tamano fijo, y luego los motores corren en TypeScript (la FFT, la DCT por bloques, el lifting wavelet, el matching pursuit ortogonal) o en la GPU como shaders de fragmento WebGL2 (el paso hacia adelante de SIREN y CPPN, evaluado por pixel). El resultado se dibuja en un canvas.\n\nComo el mismo codigo PSNR/SSIM corre en vivo y offline, una reconstruccion en vivo y su gemela horneada obtienen el mismo numero.',
    },
    {
      id: 'deploy',
      en: 'Deploy',
      ...
      body_es:
        'ImageLab se despliega como sitio estatico en GitHub Pages. Un flujo de GitHub Actions construye la aplicacion de pagina unica y superpone los artefactos derivados versionados en el sitio, luego publica al CDN detras de imglab.fasl-work.com. No hay backend: todo lo que la pagina necesita se obtiene como artefacto estatico o se calcula en vivo en tu pestana.\n\nEl horneado offline deliberadamente no corre en el despliegue; los artefactos versionados bajo data/derived son las entradas, asi que un despliegue es rapido y no puede desviarse de lo revisado.',
    },
  ],
};
