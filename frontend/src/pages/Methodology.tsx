    ...
    },
    {
      ...
      content: (
        <div className="il-doc" style={{ margin: 0 }}>
          ...
          <p>
            {t(
              'In the App each image is fitted as an explicit closed-form trigonometric equation (ridge regression on random Fourier features) that you can read term by term and export in full, and the dominant contour of the image is written exactly as its Fourier-descriptor equation. Compact formulas are the readable pole of the spectrum; the one genuinely exact case of image-to-equation is the contour, because it is a one-dimensional closed curve.',
              'En la App cada imagen se ajusta como una ecuación trigonométrica explícita de forma cerrada (regresión ridge sobre características de Fourier aleatorias) que puedes leer término a término y exportar completa, y el contorno dominante de la imagen se escribe exactamente como su ecuación de descriptores de Fourier. Las fórmulas compactas son el polo legible del espectro; el único caso genuinamente exacto de imagen-a-ecuación es el contorno, porque es una curva cerrada unidimensional.',
            )}
          </p>
    ...
    },
    {
      ...
      content: (
        <div className="il-doc" style={{ margin: 0 }}>
          ...
          <p>
            {t('A variational or adversarial model gives the image a low-dimensional latent code whose disentangled directions correspond to human-meaningful attributes: this is the most editable representation. In the App, the selected image is passed through a real VAE (its reconstruction and its latent under increasing noise) and regenerated by a real diffusion model image-to-image at increasing strength ',
              'Un modelo variacional o adversarial da a la imagen un código latente de baja dimensión cuyas direcciones desenredadas corresponden a atributos con sentido humano: es la representación más editable. En la App, la imagen seleccionada pasa por un VAE real (su reconstrucción y su latente bajo ruido creciente) y es regenerada por un modelo de difusión real imagen-a-imagen con fuerza creciente ')}
