// Per-panel error boundary: one broken representation panel must never blank the whole workbench.
...
export class PanelBoundary extends Component<Props, State> {
  ...
  componentDidUpdate(prev: Props) {
    // A new child tree (tab or image switch) gets a fresh chance instead of a sticky fallback.
  ...
  render() {
    ...
    if (error) {
      return (
        <div className="il-panel" role="alert">
          <div className="il-panel-t">{this.props.es ? 'Panel no disponible' : 'Panel unavailable'}</div>
