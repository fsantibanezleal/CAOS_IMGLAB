...
\newcommand{\versiondoi}{10.5281/zenodo.22822866}
\newcommand{\conceptdoi}{10.5281/zenodo.21510992}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_IMGLAB}
...
CAOS open-research programme \quad$\cdot$\quad CAOS\_IMGLAB, mathematical representations of images\\[2pt]
...
\author{%
  ...
  \thanks{Code, the derived artifacts, and the interactive lab are released under MIT at
    \url{https://github.com/fsantibanezleal/CAOS_IMGLAB}. Every external engine and reference
    carries its own license and citation; the explicit formulas shown are the author's own.}%
}
...
coordinates. We give the twelve families with their governing equations and the cross-family
benchmark on six images. The scope is bounded: arbitrary photographs do not reduce to compact,
faithful, human-readable equations, and the results say where a representation's parameters are, and
are not, a usable handle.
\end{abstract}
...
(iii) the U-shaped editability finding and its scope.
...
is a readable equation~\cite{stanley2007}. Fidelity is bounded: a smooth gradient reaches
...
\caption{The benchmark. Left: peak signal-to-noise ratio at a fixed parameter budget across the
twelve families; fidelity is comparable ($27$--$35$\,dB) and does not distinguish them, coloured by
pole (designed-structure local, global or entangled, learned manifold). Right: the
editability-locality metric for the four transform families; a single coefficient of a global
transform is spatially spread (Fourier $0.16$, DCT $0.23$) while a coefficient of a space-localized
transform is essentially local (wavelet $0.999$, KLT $1.0$). Data: \texttt{\_bench/index.json}.}
\label{fig:bench}
\end{figure*}

We compute each representation of six benchmark images (a photograph, a painting, a mathematical-art
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.9\textwidth]{fig-ushape.pdf}
...
models. The scope is bounded: arbitrary photographs do not reduce to compact, faithful,
human-readable equations, and the symbolic and formula families make bounded partial
reconstructions (a smooth gradient fits to $\approx57$\,dB, a hard edge to $\approx14$), which the
benchmark reports as partial results. The generative-latent pole is
...
code and artifacts are released, so the benchmark and the figures are reproducible.
...
{\raggedright
Code, the benchmark artifacts, and the interactive lab are at
...
carries its citation in the repository. Figure~\ref{fig:bench} is regenerated deterministically
from \path{_bench/index.json} by
\path{manuscripts/image-representation-editability/figures/make_figs.py}; the U-shape schematic is
hand-authored (\path{fig-ushape.svg}).\par}
...
values are stored in \path{data/derived/_bench/index.json}.
